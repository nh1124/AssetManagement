...
Use when: 利回り別のシナリオ比較`,
  ...
  server.registerTool(
    ...
    {
      title: "全口座合算プロジェクション",
      description: `現在のポートフォリオ設定をベースに、全口座（NISA・DC・持株会）の合算将来価値を試算します。
...
Use when: リタイア時・ライフイベント時の資産見込みを確認したいとき`,
    ...
    async ({ target_age, nisa_rate_pct = 5.0, dc_rate_pct = 5.0, holdings_rate_pct = 3.0 }) => {
      try {
        const p = getPortfolio();
        const yearsRemaining = target_age - p.profile.age;
        if (yearsRemaining <= 0) {
          return { content: [{ type: "text", text: `Error: target_age (${target_age}) は現在の年齢 (${p.profile.age}) より大きい値を指定してください。` }] };
        }

        const accounts = [
          { account: "NISA（積立）", monthly_amount: p.investments.nisa.monthly_amount, annual_rate_pct: nisa_rate_pct },
          { account: "企業型DC", monthly_amount: p.investments.dc.monthly_amount, annual_rate_pct: dc_rate_pct },
          { account: "NTT持株会", monthly_amount: p.investments.holdings.monthly_amount, annual_rate_pct: holdings_rate_pct }
        ];
...
Use when: DCの節税メリットを確認したいとき`,
      inputSchema: z.object({
        monthly_contribution: z.number().int().min(1).optional().describe("月額拠出（円）"),
    ...
    async ({ monthly_contribution, income_tax_rate_pct = 10, resident_tax_rate_pct = 10 }) => {
      try {
        const p = getPortfolio();
        const monthly = monthly_contribution ?? p.investments.dc.monthly_amount;
  ...
  server.registerTool(
    "calc_nisa_cap_usage",
    {
      ...
      description: `現在の月額積立でNISA生涯投資枠（1,800万円）の消化年数を試算します。

Use when: NISA枠消化のペース確認`,
      inputSchema: z.object({
        monthly_amount: z.number().int().min(1).optional().describe("月額積立（円）"),
        already_used: z.number().int().min(0).optional().default(0).describe("既使用額（円）")
      }).strict(),
      annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }
    },
    async ({ monthly_amount, already_used = 0 }) => {
      try {
        const p = getPortfolio();
        const monthly = monthly_amount ?? p.investments.nisa.monthly_amount;
        const annual = monthly * 12;
        const lifetimeCap = p.investments.nisa.lifetime_cap ?? 18_000_000;
        const annualCap = p.investments.nisa.annual_cap ?? 1_200_000;
        const remaining = lifetimeCap - already_used;
        const yearsToFill = remaining / annual;
        const fillYear = new Date().getFullYear() + Math.ceil(yearsToFill);
        const fillAge = p.profile.age + Math.ceil(yearsToFill);
        const exceedsAnnualCap = annual > annualCap;

        const result = { monthly_amount: monthly, annual_amount: annual, lifetime_cap: lifetimeCap, annual_cap: annualCap, already_used, remaining_cap: remaining, years_to_fill: +yearsToFill.toFixed(1), fill_year: fillYear, fill_age: fillAge, exceeds_annual_cap: exceedsAnnualCap, warning: exceedsAnnualCap ? `月額${monthly.toLocaleString()}円（年${annual.toLocaleString()}円）はNISA年間上限${annualCap.toLocaleString()}円を超えています` : null };

        return {
          content: [{ type: "text", text: `📋 NISA枠シミュレーション\n月額: ${monthly.toLocaleString()}円（年${(annual / 10000).toFixed(0)}万円）\n生涯枠残余: ${(remaining / 10000).toFixed(0)}万円\n枠消化: 約${yearsToFill.toFixed(1)}年後（${fillYear}年・${fillAge}歳）${exceedsAnnualCap ? "\n⚠️ " + result.warning : ""}\n\n${JSON.stringify(result, null, 2)}` }],
          structuredContent: toStructured(result)
        };
      } catch (err) {
        return { content: [{ type: "text", text: `Error: ${err instanceof Error ? err.message : String(err)}` }] };
